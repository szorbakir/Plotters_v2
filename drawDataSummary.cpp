...
//MC spectrum file
// TString MCFile = "datas/2016/MC/STD+GEN/output-MC-2a.root";
// TString MCFile = "datas/2017/MC/STD+GEN/output-MC-2a.root";
...
void drawDataSummary()
{
  ...
  TLegend *leg = new TLegend(0.60, 0.66, 0.72, 0.87);
  leg->SetBorderSize(0);
  leg->SetLineColor(0);
  leg->SetLineStyle(1);
  leg->SetLineWidth(0);
  ...
  TLegend *leg2 = new TLegend(0.72, 0.66, 0.84, 0.87);
  ...
  { //etaCounter - 1
    ...
    if (i == 0)
    {
      baseMassHist[i]->Draw();
      //* Settin some styles
      baseMassHist[i]->SetMinimum(1e-4);
      baseMassHist[i]->SetMaximum(1e20);
    ...
    { // trgCounter - 1
      ...
      // Set size for markers
      if (i == 0)
        jetTrgMassHist[j]->SetMarkerSize(1.1);
      if (i == 1)
        jetTrgMassHist[j]->SetMarkerSize(1.1);
      if (i == 2)
        jetTrgMassHist[j]->SetMarkerSize(1.3);
      if (i == 3)
    ...
    baseMassHist[i]->GetXaxis()->SetTitle("M_{jj} (GeV)");
    baseMassHist[i]->GetYaxis()->SetTitle("A.U.");
    baseMassHist[i]->GetYaxis()->SetTitle("A.U.");
    baseMassHist[i]->GetXaxis()->SetTitleOffset(1.4);
    baseMassHist[i]->GetYaxis()->SetTitleOffset(1.4);
  }
  ...
  t2->DrawLatex(0.74, 0.92, latexRight);
