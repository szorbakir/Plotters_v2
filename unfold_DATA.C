...
void unfold_DATA()
{
    ...
    TFile *Unfolded_file = new TFile("NoReg_0.0_yDATA/unfoldedSpectra-Mjj-Tau_0.0.root");
    TFile *Gen_file = new TFile("output-MC-2a.root");
    //TFile *Unfolded_file = new TFile("massSpectrums2016_halfbinned.root");
    //TFile *Gen_file = new TFile("output-MC-2a.root");
    ...
    { //
        ...
        //Gen_level[i] = (TH1D*) Gen_file->Get("Standard/Eta_" + eta[i] + "-" + eta[i+1] +"/mc"+"/hdjmass_half");
        //Unfolded_level[i] = (TH1D*) Unfolded_file->Get("massSpectrum_" +eta[i] + "-" + eta[i+1]);
        auto Scale = Gen_level[i]->Integral() / Unfolded_level[i]->Integral();
        Unfolded_level[i]->Scale(Scale);
        ...
        //leg->AddEntry(Gen_level[i],"MC_{reco}","l");
        //leg->AddEntry(Unfolded_level[i],"DATA","l");
        leg->Draw();
        ...
        Ratio->SetYTitle("MC_{Truth}/Unfolded");
        //Ratio->SetYTitle("MC_{reco}/Data");
        ...
        //c1->SaveAs("Data_vsMCreco_"+ eta[i] + "-" + eta[i+1] +".pdf");
        c1->SaveAs("Data_unfoldedvsMCgen_" + eta[i] + "-" + eta[i + 1] + ".pdf");
        leg->Clear();
        c1->Update();
